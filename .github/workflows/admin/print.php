<?php
	...
	function generateRow($conn){
        ...
        while($row = $query->fetch_assoc()){
        	...
        	$contents .= '
        		<tr>
        			<td colspan="2" align="center" style="font-size:15px;"><b>'.$row['pname'].'</b></td>
        		</tr>
        		<tr>
        	...
        	$sql = "SELECT * FROM candidates WHERE id = '$id' ORDER BY fullname ASC";
    		$cquery = $conn->query($sql);
    		while($crow = $cquery->fetch_assoc()){
    			$sql = "SELECT * FROM votes WHERE vote_id = '".$crow['id']."'";
      			$vquery = $conn->query($sql);
      			$votes = $vquery->num_rows;

      			$contents .= '
      				<tr>
      					<td>'.$crow['fullname'].", ".$crow['fullname'].'</td>
      					<td>'.$votes.'</td>
      				</tr>
      			';
